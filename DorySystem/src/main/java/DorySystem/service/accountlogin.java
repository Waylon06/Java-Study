    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (i == 1){
                ...
                writer.write("<script>alert('添加成功!');window.location.href='index.jsp'</script>");
//                req.getRequestDispatcher("/main.jsp").forward(req,resp);
            } else {
                writer.write("<script>alert('添加失败!');window.location.href='login.jsp'</script>");
//                req.getRequestDispatcher("/login.jsp").forward(req,resp);
            }
        }catch (Exception e){
            PrintWriter writer = resp.getWriter();
            writer.write("<script>alert('哦豁出错了!');window.location.href='login.jsp'</script>");
//            req.getRequestDispatcher("/login.jsp").forward(req,resp);
        }
